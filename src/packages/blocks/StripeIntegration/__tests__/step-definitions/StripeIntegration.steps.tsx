...
defineFeature(feature, (test) => {
  ...
  test("User navigates to StripeIntegration", ({ given, when, then }) => {
    ...
    then("I can enter card number", () => {
      ...
      let formattedCard = cardNumberFormatter(
        "42424",
        instance.state.cardNumber
      );
    ...
    then("close payment alert", async () => {
      ...
      const { getByTestId } = render(
        <PaymentCustomeAlert
          visible={true}
          onpressClose={mockOnClick}
          onpressContinue={mockOnSClick}
          customeText={"Payment Failed"}
          customeDescription={
            "You earnd a discount coupon code. You can check this out in your profile or Reed Now!"
          }
          isLoading={false}
          testID={"paymentAlert"}
    ...
    then("Payment all texts", async () => {
      ...
      if (instance.state.paymentAlerttype === "PaymentSuccess") {
        instance.setState({ paymentAlerttype: "ThankYouForYourOder" }, () => {
          console.log(
            "check paymetn alert type--->",
            instance.state.paymentAlerttype
          );
          expect(instance.state.paymentAlerttype).toEqual(
            "ThankYouForYourOder"
          );
    ...
    then(
      'should change paymentAlerttype to "ThankYouForYourOder" and call handlePaymentSuccess',
      () => {
        let explorePageWrapper: ShallowWrapper;
        explorePageWrapper = shallow(
          <StripeIntegration
        ...
        // Check state changes
        expect(explorePageWrapper.state("paymentAlerttype")).toBe(
          "ThankYouForYourOder"
        );
    ...
    then(
      'should change paymentAlerttype to "ContinueToEmail" and call handlePaymentSuccess',
      () => {
        let explorePageWrapper: ShallowWrapper;
        explorePageWrapper = shallow(
          <StripeIntegration
        ...
        explorePageWrapper.setState({
          isOrderSuccess: true,
          paymentAlerttype: "ThankYouForYourOder",
        });
    ...
    then(
      "should navigate to InvoiceBilling and set showPaymentAlert to false for unexpected paymentAlerttype",
      () => {
        let explorePageWrapper: ShallowWrapper;
        explorePageWrapper = shallow(
          <StripeIntegration
        ...
        explorePageWrapper.setState({
          isOrderSuccess: true,
          paymentAlerttype: "UnexpectedValue",
        });
    ...
    then(
      "should set showPaymentLoading to true and call getPaymentMethod for paymentMethodType as Card",
      () => {
        let explorePageWrapper: ShallowWrapper;
        explorePageWrapper = shallow(
          <StripeIntegration
        ...
        // Click on Button 1
        explorePageWrapper.find(
          <DoubleButton
            button1Label={""}
            button2Label={""}
            button1_Onpress={mockGetPaymentMethod()}
            button2_Onpress={() => {}}
          />
        );
        ...
        expect(explorePageWrapper.state("customAlertText")).toBe(
          "Paymenst In Process.."
        );
    ...
    then(
      'should call codeApiCalled and set showPaymentAlert to true when paymentMethodType is not "Card"',
      () => {
        let explorePageWrapper: ShallowWrapper;
        explorePageWrapper = shallow(
          <StripeIntegration
        ...
        explorePageWrapper.find(
          <DoubleButton
            button1Label={""}
            button2Label={""}
            button1_Onpress={mockCodeApiCalled()}
            button2_Onpress={() => {}}
          />
        );
        const { getByTestId } = render(
    ...
    then("Double button", async () => {
      ...
      expect(instance.state.showPaymentAlert).toEqual(true);
      let card = instance.state.cardNumber
        .replace(" ", "")
        .replace(" ", "")
        .replace(" ", "");
    ...
    then(
      'should set custom alert text and description for "Card" payment method type',
      () => {
        let explorePageWrapper: ShallowWrapper;
        explorePageWrapper = shallow(
          <StripeIntegration
        ...
        instance.setState({
          customAlertDesc: "Please contact to admin Or Try again.",
        });
    ...
    then("stripe payment api", async () => {
      ...
      let url = await instance.getPaymentMethod(
        cardNumber,
        cvv,
        expMonth,
        expYear,
      ...
      expect(url).toBe(
        `https://api.stripe.com/v1/payment_methods?card[number]=${cardNumber}&card[exp_month]=${expMonth}&card[exp_year]=${expYear}&card[cvc]=${cvv}&type=card`
