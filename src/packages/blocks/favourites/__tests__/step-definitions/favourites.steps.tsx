...
defineFeature(feature, (test) => {
    ...
    test('User can view any favourites', ({ given, when, then }) => {
        ...
        then('I can view favourites will load with out errors', () => {
            FavouritesWrapper = shallow(<Favourites {...screenProps} />)
            const touchableOpacity = FavouritesWrapper.find(
                '[testID="btnDeleteFavorite"]'
            );
            touchableOpacity.simulate("press");

            const closeFavouriteTouchablity = FavouritesWrapper.find(
    ...
    test('title and content', ({ given, when, then }) => {
        ...
        then('add favourites action', () => {
            const touchableOpacity = AddInteractiveFavouritessWrapper.find(
                '[testID="Background"]'
            );
            touchableOpacity.simulate("press");

            const touchableOpacitys = AddInteractiveFavouritessWrapper.find(
                '[testID="btnAddFavoriteTxt"]'
            );
            touchableOpacitys.simulate("press");


            let textInputComponent = AddInteractiveFavouritessWrapper.findWhere(
                (node) => node.prop("testID") === "txtInputType"
            );
            const event = {
                preventDefault() { },
                target: { value: "AccountBlock::Account" },
            };
            textInputComponent.simulate("change", event);

            // const { getByTestId } = render(<AddFavourites {...screenProps} />);
            // const input = getByTestId('txtInputId');
        ...
        then('add TextInput action', () => {
            ...
            // instance = AddInteractiveFavouritessWrapper.instance() as AddFavourites;
            // instance.setState({ favouriteId : "1" })
            // expect(input.props.value).toBe('1');

            // let textInputComponents = AddInteractiveFavouritessWrapper.findWhere(
            //     (node) => node.prop("testID") === "txtInputId"
            // );
            // const events = {
            //     preventDefault() { },
            //     target: { value: "1" },
            // };
            // textInputComponents.simulate("change", events);
        });
